...
RUN pip install gym tensorflow sacred pymongo matplotlib GitPython cython lockfile glfw imageio gym[mujoco] cffi
...
ENV MUJOCO_PY_MJPRO_PATH /.mujoco/mjpro150
WORKDIR /
RUN mkdir -p /.mujoco \
...
RUN useradd -d /project -u 197614 --create-home user 
USER user
WORKDIR /project/
ADD . /project/
